...
public class SidePanelTest {
    ...
    public void testPaintComponentDrawsStatisticsAndControls(){
        ...
        when(mockGame.getFruitsEaten()).thenReturn(5);          // TODO: Andrew will comment the rest of these lines
        when(mockGame.getNextFruitScore()).thenReturn(20);

        when(mockGraphics.getFontMetrics()).thenReturn(mockFontMetrics);
        when(mockFontMetrics.stringWidth("Snake Game")).thenReturn(100);

        SidePanel panel = new SidePanel(mockGame);
        ...
        verify(mockGraphics).drawString("Total Score: 100", 50, 180);
        verify(mockGraphics).drawString("Fruit Eaten: 5", 50, 210);
        verify(mockGraphics).drawString("Fruit Score: 20", 50, 240);

        // Assert controls text was drawn
        verify(mockGraphics).drawString("Move Up: W / Up Arrowkey", 50, 350);
        verify(mockGraphics).drawString("Move Down: S / Down Arrowkey", 50, 380);
        verify(mockGraphics).drawString("Move Left: A / Left Arrowkey", 50, 410);
        verify(mockGraphics).drawString("Move Right: D / Right Arrowkey", 50, 440);
        verify(mockGraphics).drawString("Pause Game: P",50,480);
    }
}
